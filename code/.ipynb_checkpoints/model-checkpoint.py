...
class NGCF(BasicModel):
    def __init__(self,config:dict,dataset:BasicDataset):
        ...
        self.config = config
        self.dataset: dataloader.BasicDataset=dataset
        self.weight_dict = self.__init_weight()
    
    def __init_weight(self):
        self.n_user = self.dataset.n_users
        self.m_item = self.dataset.m_items
        self.latent_dim = self.config['latent_dim_rec']
        # self.n_layers = self.config['lightGCN_n_layers']
        ...
        self.embedding_user = torch.nn.Embedding(num_embeddings=self.n_user, embedding_dim=self.latent_dim)
        self.embedding_item = torch.nn.Embedding(num_embeddings=self.m_item, embedding_dim=self.latent_dim)
        nn.init.normal_(self.embedding_user.weight, std=0.1)
        nn.init.normal_(self.embedding_item.weight, std=0.1)
        world.cprint('use NORMAL distribution initilizer')
        self.f = nn.Sigmoid()
        ...
        layers = [self.latent_dim] + self.layers
    ...
    def computer(self):
        ...
        embs = [all_emb]
        for k in range(len(self.layers)):
            side_embeddings = torch.sparse.mm(A_hat,all_emb)
            ...
            # message dropout.
            all_emb = nn.Dropout(self.mess_dropout[k])(all_emb)

            # normalize the distribution of embeddings.
            norm_embeddings = F.normalize(all_emb, p=2, dim=1)

            embs += [norm_embeddings]

        embs = torch.cat(embs, 1)
        users = embs[:self.n_user, :]
        items = embs[self.n_user:, :]
